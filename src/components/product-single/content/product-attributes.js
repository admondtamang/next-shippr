...
export const ProductAttributes = ({ className = "mb-4", title, attributes, active, onClick }) => {
        ...
        <div className={className}>
            ...
            <ul className="colors flex flex-wrap -me-3">
                {attributes?.map(({ id, value, meta }) => (
                    ...
                    >
                        {title === "color" ? <span className="h-full w-full rounded block" style={{ backgroundColor: meta }} /> : value}
                    </li>
                ))}
            </ul>
        </div>
